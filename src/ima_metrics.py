...
def compute_area (): #in mm2
    ...
    if cfg.MVMU_ver == "Analog":
        area += (cfg.num_matrix * (3 + datacfg.ReRAM_xbar_num)) * cfg.xbar_size * param.dac_area # 1 dac for input of f/b/d xbars, each phy xbar in d-xbar will have a dac_array, hence 8
        area += (cfg.num_matrix * 2 * 2 * datacfg.ReRAM_xbar_num) * cfg.xbar_size * param.snh_area # snh for f/b xbars, also for both positive and negative xbar
        area += (cfg.num_matrix * 2) * param.sna_area # sna for one each f/b xbars
        area += cfg.num_adc * param.adc_area # adc
        area += (cfg.num_matrix * 3) * param.xbar_outMem_area # xbar_outMem (1 OR for 8 xbars - 16 bit weights)
    ...
    area += (cfg.num_matrix * 3) * param.xbar_inMem_area # xbar_inMem one each for f/b/d xbars, 16 bit
...
def compute_pow_dyn ():
    ...
    # dyn_pow += cfg.num_xbar/2 * 1.2 # (adding dyn pow the way issac does for comparison)
        dyn_pow += (cfg.num_matrix * (3 + datacfg.ReRAM_xbar_num)) * cfg.xbar_size * param.dac_pow_dyn # dac
        dyn_pow += (cfg.num_matrix * 2 * 2 * datacfg.ReRAM_xbar_num) * cfg.xbar_size * param.snh_pow_dyn # snh
        ...
        for bits_per_cell in datacfg.storage_config:
            if (bits_per_cell != 's'):
                dyn_pow += (cfg.num_matrix * 4) * param.xbar_ip_pow_dyn_dict[bits_per_cell] # xbar ip power considred as ip>op power
    else:
        dyn_pow += (cfg.num_matrix * 2) * param.xbar_ip_pow_dyn_dict # xbar ip power considred as ip>op power # d-xbar are not needed in Digital MVMUs only f and b are there
